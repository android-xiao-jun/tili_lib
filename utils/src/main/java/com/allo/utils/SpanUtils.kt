...
class SpanUtils() {
     ...
     * @return the single [SpanUtils] instance
     */
    fun setClickSpan(@ColorInt color: Int, underlineText: Boolean=false, listener: View.OnClickListener?): SpanUtils {
        if (mTextView != null && mTextView!!.movementMethod == null) {
            mTextView!!.movementMethod = LinkMovementMethod.getInstance()
        }
...
//            LogUtils.e(fm, sfm);
    ...
    internal class CustomImageSpan : CustomDynamicDrawableSpan {
        ...
        constructor(b: Bitmap, verticalAlignment: Int) : super(verticalAlignment) {
            mDrawable = BitmapDrawable(Utils.getApp().resources, b)
            mDrawable?.run {
            ...
            get() {
                ...
                when {
                    ...
                    mContentUri != null -> {
                        val bitmap: Bitmap
                        try {
                            val `is` = Utils.getApp().contentResolver.openInputStream(mContentUri!!)
                            bitmap = BitmapFactory.decodeStream(`is`)
                            drawable = BitmapDrawable(Utils.getApp().resources, bitmap)
                            drawable.setBounds(
...
//                    "lineHeight: " + (bottom - top));
    ...
    companion object {
        ...
        fun with(textView: TextView? = null): SpanUtils {
